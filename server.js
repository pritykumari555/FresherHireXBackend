...
app.post("/login", async (req, res) => {
  try {
    ...
    if (!ExistUser) return res.status(404).json({ message: "No account found ❌" });
    if (ExistUser.password !== password) return res.status(401).json({ message: "Wrong password ❌" });
...
app.post("/apply", async (req, res) => {
  try {
    ...
    const newApp = await Application.create({ name, email, skill, projectLink, github });
...
app.post("/question", async (req, res) => {
  try {
    ...
    const newQ = await Question.create({ company: company.toLowerCase(), question, options, answer });
